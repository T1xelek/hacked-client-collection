...
import incest.tusky.game.module.impl.Combat.BackTrack;
...
public class EntityRenderer implements IResourceManagerReloadListener {
  ...
  public void getMouseOver(final float partialTicks) {
                ...
                if (entity != null && this.mc.world != null) {
                    ...
                    Vec3d vec33 = null;
                    final float f = 1.0F;
                    final List<Entity> list = this.mc.world.getEntitiesInAABBexcluding(entity, entity.getEntityBoundingBox().addCoord(vec31.x * d0, vec31.y * d0, vec31.z * d0).expand(f, f, f), Predicates.and(EntitySelectors.NOT_SPECTATING, Entity::canBeCollidedWith));
                    double d2 = d1;
                    ...
                    if (BackTrack.target != null) {
                        for (final Vec3d positions : BackTrack.positions) {
                            final float f1 = 0.1f;

                            AxisAlignedBB axisalignedbb = new AxisAlignedBB(positions.x - 0.3, positions.y, positions.z - 0.3, positions.x + 0.3, positions.y + 1.8, positions.z + 0.3);
                    ...
                    if (pointedEntity != null && flag && vec3.distanceTo(vec33) > reach) {
                        pointedEntity = null;
                        this.mc.objectMouseOver = new RayTraceResult(RayTraceResult.Type.MISS, vec33, (EnumFacing) null, new BlockPos(vec33)); }

                    if (pointedEntity != null && (d2 < d1 || this.mc.objectMouseOver == null)) {
                        this.mc.objectMouseOver = new RayTraceResult(pointedEntity, vec33);
