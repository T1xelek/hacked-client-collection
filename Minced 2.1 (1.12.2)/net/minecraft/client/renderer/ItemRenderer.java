...
public class ItemRenderer {
    ...
    private void renderArmFirstPerson(float p_187456_1_, float p_187456_2_, EnumHandSide p_187456_3_) {
        EventViewModel event = new EventViewModel(p_187456_3_);
    ...
    private void transformEatFirstPerson(float p_187454_1_, EnumHandSide p_187454_2_, ItemStack p_187454_3_) {
        int i;
        EventViewModel event = new EventViewModel(p_187454_2_);
        EventManager.call(event);
    ...
    private void transformSideFirstPerson(EnumHandSide p_187459_1_, float p_187459_2_) {
        EventViewModel event = new EventViewModel(p_187459_1_);
        EventManager.call(event);
    ...
    private void transformSideFirstPerson2(EnumHandSide enumHandSide, float p_187459_2_) {
        EventViewModel event = new EventViewModel(enumHandSide);
        EventManager.call(event);
        ...
        if (tuskevich.instance.featureManager.getFeature(SwingAnimations.class).isEnabled() && SwingAnimations.item360.getCurrentValue() && !(mc.player.isSwingInProgress && SwingAnimations.stopInSwing.getCurrentValue())) {
            ...
            GlStateManager.rotate(angle, SwingAnimations.item360Mode.currentMode.equals("Vertical") ? -angle : 0.0f, SwingAnimations.item360Mode.currentMode.equals("Horizontal") ? 1.0f : 0.0f, 0.0f);
        }
    }

    private void transformFirstPersonItem(final float equipProgress, final float swingProgress) {
        GlStateManager.translate(ViewModel.rightX.getCurrentValue(), ViewModel.rightY.getCurrentValue(), ViewModel.rightZ.getCurrentValue());
                        ...
                        if (tuskevich.instance.featureManager.getFeature(SwingAnimations.class).isEnabled() && !SwingAnimations.auraOnly.getCurrentValue() || tuskevich.instance.featureManager.getFeature(SwingAnimations.class).isEnabled() && tuskevich.instance.featureManager.getFeature(KillAura.class).isEnabled() && KillAura.target != null) {
                            if (enumhandside != (this.mc.gameSettings.mainHand.equals(EnumHandSide.LEFT) ? EnumHandSide.RIGHT : EnumHandSide.LEFT)) {
                                if (mode.equalsIgnoreCase("Default")) {
                                    ...
                                    GlStateManager.rotate(var4 * -20.0f, 0.0f, 0.0f, 2.0f);
                                    GlStateManager.rotate(var4 * -75.0f, 1.0f, 0.0f, 0.0f);
                                } else if (mode.equalsIgnoreCase("Liquid")) {
                                    this.transformSideFirstPerson2(enumhandside, p_187457_7_);
                                    translate3();
                                    float var3 = MathHelper.sin(swingprogress * swingprogress * 3.1415927f);
                                    float var4 = MathHelper.sin(MathHelper.sqrt(swingprogress) * 3.1415927f);
                                    ...
                                    GlStateManager.rotate((float) (System.currentTimeMillis() / 16L * (long) ((int) SwingAnimations.spinSpeed.getCurrentValue()) % 360L), 0.0f, 0.0f, -0.1f);
