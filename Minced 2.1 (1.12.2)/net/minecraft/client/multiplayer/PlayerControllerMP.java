...
public class PlayerControllerMP implements IPlayerControllerMP {
    ...
    public EnumActionResult processRightClick(EntityPlayer player, World worldIn, EnumHand stack) {
        if (this.currentGameType == GameType.SPECTATOR) {
            ...
            this.syncCurrentPlayItem();
            if (!player.getCooldownTracker().hasCooldown(player.getHeldItem(stack).getItem()))
     ...
     */
            ...
            connection.sendPacket(new CPacketEntityAction(mc.player, CPacketEntityAction.Action.STOP_SPRINTING));
            this.syncCurrentPlayItem();
