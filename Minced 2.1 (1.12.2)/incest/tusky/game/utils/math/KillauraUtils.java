package incest.tusky.game.utils.math;

import incest.tusky.game.tuskevich;
import incest.tusky.game.module.impl.Combat.AntiBot;
import incest.tusky.game.module.impl.Combat.AutoTotem;
...
import static incest.tusky.game.module.impl.Combat.KillAura.watercrits;
...
public class KillauraUtils implements Helper {
    ...
    public static boolean canAttack(EntityLivingBase player) {
        ...
        if ((tuskevich.instance.featureManager.getFeature(AntiBot.class).isEnabled() && AntiBot.isBotPlayer.contains(player))) {
            return false;
        }
    ...
    public static boolean isBot(EntityLivingBase entity) {
        if (entity instanceof EntityPlayer) {
            ...
            return tuskevich.instance.featureManager.getFeature(AntiBot.class).isEnabled() && AntiBot.isBotPlayer.contains(player);
        }
        return false;
    }
        ...
        if (!target.isDead) {
            if (KillAura.clickMode.currentMode.equalsIgnoreCase("1.9")) {
                ...
                if (mc.player.getCooledAttackStrength(0.5f) >= attackDelay) {
                    if (mc.player.isInWater() && watercrits.getCurrentValue()) {
