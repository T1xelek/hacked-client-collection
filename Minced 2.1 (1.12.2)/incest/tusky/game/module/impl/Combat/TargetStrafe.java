...
public class TargetStrafe extends Module {
    ...
    public void onUpdate(EventPreMotion event) {
        ...
        if (KillAura.target.getHealth() > 0) {
            if (autoJump.getCurrentValue() && tuskevich.instance.featureManager.getFeature(KillAura.class).isEnabled() && tuskevich.instance.featureManager.getFeature(TargetStrafe.class).isEnabled()) {
    ...
    public void onMove(EventMove e) {
        if (tuskevich.instance.featureManager.getFeature(KillAura.class).isEnabled() && KillAura.target != null && KillAura.target.getHealth() > 0) {
            if (mc.player.isCollidedHorizontally)
                switchDirection();
