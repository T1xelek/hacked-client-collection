...
public class DamageParticles extends Module {
    ...
    public DamageParticles() {
        super("DamageParticles", "Партиклы после нанесения урона", ModuleCategory.Visual);
    ...
    public void onRender(EventRender3D e) {
        if (particlesMode.currentMode.equals("UwU")) {
            if (timerHelper.hasReached(deleteAfter.getCurrentValue() * 1000)) {
                particles.clear();
                timerHelper.reset();
            }
            if (!particles.isEmpty()) {
                for (Particle p : particles) {
                    if (p != null) {
                        ...
                        GlStateManager.scale(-0.03, -0.03, 0.03);
                        GL11.glDepthMask(false);
                        Helper.mc.fontRendererObj.drawBlurredString("UwU", (float) (-Helper.mc.fontRendererObj.getStringWidth(p.str) * 5), -Helper.mc.fontRendererObj.FONT_HEIGHT + 1, 8, RenderUtils.injectAlpha(p.color, 100), p.color.getRGB());
                        GL11.glColor4f(1, 1, 1, 1);
                        GL11.glDepthMask(true);
                        GlStateManager.doPolygonOffset(1, 1500000);
                        GlStateManager.disablePolygonOffset();
