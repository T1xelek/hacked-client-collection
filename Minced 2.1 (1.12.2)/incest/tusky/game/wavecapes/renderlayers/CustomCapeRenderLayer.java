package incest.tusky.game.wavecapes.renderlayers;

import incest.tusky.game.module.impl.Render.BabyBoy;
...
import incest.tusky.game.wavecapes.util.Mth;
...
public class CustomCapeRenderLayer implements LayerRenderer<AbstractClientPlayer> {
    ...
    public void doRenderLayer(AbstractClientPlayer abstractClientPlayer, float paramFloat1, float paramFloat2, float deltaTick,
        ...
        if (tuskevich.instance.featureManager.getFeature((CustomModel.class)).getState()) {
            return;
        }
        ...
        if (tuskevich.instance.featureManager.getFeature(BabyBoy.class).isEnabled() && abstractClientPlayer == Minecraft.getMinecraft().player)  {
...
//        }
        this.playerRenderer.bindTexture(new ResourceLocation("minced/yfas.png"));
