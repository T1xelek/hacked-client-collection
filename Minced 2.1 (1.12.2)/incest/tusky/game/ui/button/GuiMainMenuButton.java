package incest.tusky.game.ui.button;
    ...
    public void drawButton(Minecraft mc, int mouseX, int mouseY, float mouseButton) {
        if (this.visible) {
            ...
            if (this.hovered) {
                if (this.fade < 100)
                    this.fade += 8;
                text = oneColor.getColorValueColor();
            } else {
                if (this.fade > 20)
                    this.fade -= 8;
            }
            ...
            RoundedUtil.drawRound((float)this.xPosition, (float)this.yPosition, (float)this.width, (float)height - 3, 3,   new Color(25,25,25));
            mc.Nunito16.drawString(this.displayString, this.xPosition + this.width / 2F - mc.Nunito16.getStringWidth(displayString) / 2, (this.yPosition) + (this.height - 5), text.getRGB());
