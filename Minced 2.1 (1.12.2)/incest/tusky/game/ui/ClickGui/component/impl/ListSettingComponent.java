...
public class ListSettingComponent extends ExpandableComponent implements PropertyComponent {
    ...
    public void drawComponent(ScaledResolution scaledResolution, int mouseX, int mouseY) {
        ...
        if (isExpanded()) {
            if (ClickGUI.shadow.getCurrentValue()) {
                RenderUtils.drawBlurredShadow(x + incest.tusky.game.ui.ClickGui.panelcomponent.X_ITEM_OFFSET - 2, y + height - 1, x - (x + incest.tusky.game.ui.ClickGui.panelcomponent.X_ITEM_OFFSET - 3) - 3 + width - incest.tusky.game.ui.ClickGui.panelcomponent.X_ITEM_OFFSET, y + getHeightWithExpand() - (y + height - 1), 5, new Color(25, 25, 25, 200));
            }
            Gui.drawRect(x + incest.tusky.game.ui.ClickGui.panelcomponent.X_ITEM_OFFSET - 2, y + height - 1, x - 3 + width - incest.tusky.game.ui.ClickGui.panelcomponent.X_ITEM_OFFSET, y + getHeightWithExpand(), new Color(25, 25, 25, 200).getRGB());
        }
    ...
    private void handleRender(int x, int y, int width, int textColor) {
        ...
        for (String e : listSetting.getModes()) {
            if (listSetting.currentMode.equals(e)) {
                mc.mntsb_14.drawString(e, x + incest.tusky.game.ui.ClickGui.panelcomponent.X_ITEM_OFFSET + width / 2 + 0.5f - 48, getY() + 13, -1);

                if (isExpanded()) {
                    mc.mntsb_14.drawCenteredString(e, x + incest.tusky.game.ui.ClickGui.panelcomponent.X_ITEM_OFFSET + width / 2 + 0.5f- 2, y + 0.5F, ClickGUI.color.getColorValue());
                }
            } else {
                if (isExpanded()) {
                    mc.mntsb_14.drawCenteredString(e, x + incest.tusky.game.ui.ClickGui.panelcomponent.X_ITEM_OFFSET + width / 2 + 0.5f - 2, y + 0.5F, Color.WHITE.getRGB());
                }
            }
